...
• LISTS: actively look for latent lists hiding in prose — features, benefits, steps, reasons, examples, tools. When the author names three or more parallel items, even if they wrote them as one flowing sentence or a comma-separated run, break them onto their own lines with a "• " bullet. Use only the author's words: never invent an item, never split one item into two to pad the count, and never merge distinct items. Give each list a short plain lead-in line where one fits naturally from the author's existing text. A genuine parallel set should become a bulleted list; ordinary narrative, a single example, or two items should stay as prose.
...
export function buildFormatPrompt(text) {
  return `Reformat this LinkedIn post for the feed. Restructure it into a scannable post — a standalone hook on its own line, short one-to-two-line paragraphs, and a blank line between beats — and apply light polish (capitalization, sentence punctuation, contraction apostrophes). When the author names three or more parallel items (features, benefits, steps, reasons), break them into a "• " bulleted list using only their words, even if they wrote them as flowing prose. Do not change the author's words, meaning, or order, and do not add any new content. Then add restrained emphasis using only these markers: **bold**, _italic_, __underline__, ~~strikethrough~~.
