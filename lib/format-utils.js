...
function wordTokens(text) {
  // Strip apostrophes before tokenizing so the allowed light copy-edit
  // (im → I'm, dont → don't) doesn't read as invented words against the
  // original. Genuinely new vocabulary is still caught.
  return stripStyling(text).toLowerCase().replace(/['’]/g, '').match(/[a-z0-9]+/g) || [];
}
...
export function preFormatPost(text) {
  ...
  const broken = paragraphs.map(paragraph => {
    ...
    // Split after sentence punctuation regardless of whether the next sentence
    // starts with a capital — brain-dump input is often all lowercase.
    const sentences = p.split(/(?<=[.!?])\s+(?=[A-Za-z"“])/);
...
// Peels matched wrapping quote characters (straight, smart, single, double, or
// triple) off the whole string, including when the model puts them on their own
// line. Only strips when BOTH a leading and trailing quote run are present, so a
// post that legitimately opens with a quotation (e.g. "This changed everything.")
// keeps its quote.
function stripWrappingQuotes(text) {
  const lead = /^["'“”‘’]+[ \t]*\n?/;
  const tail = /\n?[ \t]*["'“”‘’]+$/;
  let result = text.trim();
  for (let i = 0; i < 5 && lead.test(result) && tail.test(result); i++) {
    const stripped = result.replace(lead, '').replace(tail, '').trim();
...
export function sanitizeFormattedOutput(raw, original) {
  ...
  // Strip a wrapping markdown code fence (```), possibly language-tagged.
  result = result
    .replace(/^```[^\n]*\n?/, '')
    .replace(/\n?```$/, '')
    .trim();

  // Strip wrapping quotes (handles ", ', """ and smart quotes on their own line).
  result = stripWrappingQuotes(result);

  // Remove conversational lead-ins, then strip quotes once more in case a lead-in
  // sat above a quoted block.
  result = result
