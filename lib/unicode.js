...
export function stripStyling(text) {
  return [...text.replace(/\u00a0\u0332/g, ' ').replace(/[\u0332\u0336]/g, '')]
    .map(c => STYLE_REVERSE_MAP.get(c) || c)
    .join('');
}

// Regular spaces frequently drop a combining mark that's attached to them \u2014
// most renderers (LinkedIn included) treat a plain space as having no glyph
// to attach a mark to, so the underline breaks at every word boundary.
// Swapping in a non-breaking space keeps the glyph (and the mark) intact,
// so the line stays continuous across words.
export function toUnderline(text) {
  return [...text].map(c => c === ' ' ? '\u00a0\u0332' : c + '\u0332').join('');
}
...
export function toggleStyle(text, styleKey) {
  if (styleKey === 'underline') {
    const stripped = text.replace(/\u00a0\u0332/g, ' ').replace(/\u0332/g, '');
