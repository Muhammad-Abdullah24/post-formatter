...
export function toDescendingList(text) {
  const lines = text.split('\n').filter(l => l.trim());
  return lines.map((l, i) => `${lines.length - i}→ ${l}`).join('\n');
}
...
function undoTransform(text, transform) {
  const upperOutArr = [...(transform.upperOut || '')];
  const lowerOutArr = [...(transform.lowerOut || '')];
  const digitsOutArr = [...(transform.digitsOut || '')];
  
  return [...text].map(char => {
    const uIdx = upperOutArr.indexOf(char);
    const lIdx = lowerOutArr.indexOf(char);
    const dIdx = digitsOutArr.indexOf(char);
    if (uIdx > -1) return transform.upper[uIdx];
    if (lIdx > -1) return transform.lower[lIdx];
    if (dIdx > -1) return transform.digits[dIdx];
    return char;
  }).join('');
}
...
export function toggleStyle(text, styleKey) {
  ...
  const undone = undoTransform(text, transform);
  if (undone !== text) {
    return undone;
  }
  return applyTransform(text, transform);
}
